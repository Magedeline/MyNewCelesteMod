...
{
    [CustomEntity("Ingeste/MagolorDummy")]
    public class MagolorDummy : Entity
    {
        ...
        public MagolorDummy(Vector2 position, int index = 0) : base(position)
        {
            this.index = index;
        }
            ...
            : this(data.Position + offset, data.Int("index", 0))
        {
            ...
            Sprite = new PlayerSprite(PlayerSpriteMode.Magolor);
            ...
            Add(Light = new VertexLight(new Vector2(0f, -8f), Color.Purple, 1f, 20, 60));
        ...
        {
            // Allow player to climb on Magolor - provides magic boost
            if (player.StateMachine.State == global::Celeste.Player.StClimb && player.Bottom <= Top + 4f)
            {
                player.Speed.Y = Math.Min(player.Speed.Y, -125f); // magic-powered boost
