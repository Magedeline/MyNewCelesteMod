namespace DesoloZantas.Core.Core.Entities
{
	...
	public class HeartGemDisplayMod : Component
	{
		...
		public HeartGemDisplayMod(int heartgem, bool hasGem) : base(true, true)
		{
			...
			{
				// Create sprite with proper ID (heartgem0-4)
				this.Sprites[i] = GFX.GuiSpriteBank.Create("heartgem" + i);
				this.Sprites[i].Visible = (heartgem == i && hasGem);
				
				// Play the appropriate animation
				if (this.Sprites[i].Has("spin"))
				{
					this.Sprites[i].Play("spin", false, false);
				}
				else if (this.Sprites[i].Has("idle"))
				{
					this.Sprites[i].Play("idle", false, false);
				}
			}
		...
		public void Wiggle()
		{
			...
			for (int i = 0; i < this.Sprites.Length; i++)
			{
				if (this.Sprites[i].Visible && this.Sprites[i].Has("spin"))
				{
