...
{
    [CustomEntity("Ingeste/CodyDummy")]
    public class CodyDummy : Entity
    {
        ...
        public CodyDummy(Vector2 position, int index = 0) : base(position)
        {
            this.index = index;
        }

        // Standard Everest constructor for map loading
        public CodyDummy(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Int("index", 0))
        {
            this.index = index;
            
            Collider = new Hitbox(6f, 6f, -3f, -7f);
            Sprite = new PlayerSprite(PlayerSpriteMode.Cody);
            ...
            Add(Light = new VertexLight(new Vector2(0f, -8f), Color.Cyan, 1f, 20, 60));
