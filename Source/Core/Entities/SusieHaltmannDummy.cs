...
{
    [CustomEntity("Ingeste/SusieHaltmannDummy")]
    public class SusieHaltmannDummy : Entity
    {
        ...
        public SusieHaltmannDummy(Vector2 position, int index = 0) : base(position)
        {
            this.index = index;
        }

        // Standard Everest constructor for map loading
        public SusieHaltmannDummy(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Int("index", 0))
        {
            this.index = index;
            
            Collider = new Hitbox(6f, 6f, -3f, -7f);
            Sprite = new PlayerSprite(PlayerSpriteMode.SusieHaltmann);
            ...
            Add(Light = new VertexLight(new Vector2(0f, -8f), Color.Silver, 1f, 20, 60));
        ...
        {
            // Allow player to climb on SusieHaltmann - provides technology boost
            if (player.StateMachine.State == global::Celeste.Player.StClimb && player.Bottom <= Top + 4f)
            {
                player.Speed.Y = Math.Min(player.Speed.Y, -125f); // technology-powered boost
