...
{
    [CustomEntity("Ingeste/SqueakerDummy")]
    public class SqueakerDummy : Entity
    {
        ...
        public SqueakerDummy(Vector2 position, int index = 0) : base(position)
        {
            this.index = index;
        }

        // Standard Everest constructor for map loading
        public SqueakerDummy(EntityData data, Vector2 offset)
            : this(data.Position + offset, data.Int("index", 0))
        {
            this.index = index;
            
            Collider = new Hitbox(6f, 6f, -3f, -7f);
            Sprite = new PlayerSprite(PlayerSpriteMode.Squeaker);
            ...
            Add(Light = new VertexLight(new Vector2(0f, -8f), Color.Gray, 1f, 20, 60));
        ...
        {
            // Allow player to climb on Squeaker - provides squeak boost
            if (player.StateMachine.State == global::Celeste.Player.StClimb && player.Bottom <= Top + 4f)
            {
                player.Speed.Y = Math.Min(player.Speed.Y, -125f); // squeak-powered boost
