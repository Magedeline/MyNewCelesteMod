namespace DesoloZantas.Core.Core.Cutscenes
{
    ...
    /// Handles the sequence where Oshiro has his emotional breakdown
    /// </summary>
    public class CS05_OshiroBreakdown(global::Celeste.Player player, NPC oshiro) : CutsceneEntity
    {
        public const string Flag = "oshiro_breakdown";

        private const int PlayerWalkTo = 200;

        private readonly List<DustStaticSpinner> creatures = [];

        private readonly List<Vector2> creatureHomes = [];

        private readonly NPC oshiro = oshiro;

        private readonly global::Celeste.Player player = player;

        private Vector2 origin = oshiro.Position;
        ...
        private IEnumerator MoveDust(DustStaticSpinner creature, Vector2 to)
        {
            ...
            SimpleCurve curve = new(vector, to, (to + vector) / 2f + Vector2.UnitY * (-30f + Calc.Random.NextFloat(60f)));
            for (float p = 0f; p < 1f; p += Engine.DeltaTime)
            {
                yield return null;
